...
try
    ...
    nanRows = any(isnan(ohlc_data), 2); % Check for NaNs in any OHLC column
...
% ---
% IMPORTANT:
% 1. Download the .onnx file from Hugging Face and place it here.
% 2. Install the "Deep Learning Toolbox Interface for YOLO v8" Add-On.
% ---
modelFile = 'stockmarket-pattern-detection-yolov8.onnx';
modelLoaded = false;
if ~isfile(modelFile)
    warning('YOLOv8 model file not found: %s', modelFile);
    warning('Download from: https://huggingface.co/foduucom/stockmarket-pattern-detection-yolov8');
else
    try
...
% Note: This plot logic is simplified. It shows all 'buy' signals
% even if a trade wasn't taken (e.g., already in position).
% A more accurate plot would use the 'positions' vector.
...
legend('Price', 'SMA Fast', 'SMA Slow', 'Enter Long', 'Exit Long', 'Location', 'northwest');
ylabel('Price ($)');
xlabel('Date');
grid on;
